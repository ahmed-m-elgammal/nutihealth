...
export const buildProgramInsight = (program: WorkoutProgram): ProgramInsight => {
    ...
    const focusAreas = Array.from(new Set(workoutDays.flatMap((day) => day.focus)));
    ...
    return {
        ...
        totalSetsPerWeek,
        averageSessionDuration: workoutDays.length > 0 ? Math.round(totalDuration / workoutDays.length) : 0,
        focusAreas,
