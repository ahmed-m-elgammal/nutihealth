...
export async function getDietTemplates(): Promise<Diet[]> {
    try {
        ...
        return await dietsCollection.query(Q.where('type', 'preset')).fetch();
    } catch (error) {
 ...
 */
export async function activateDiet(userId: string, dietId: string, startDate: Date = new Date(), endDate?: Date): Promise<UserDiet> {
    try {
    ...
    try {
        ...
        await database.write(async () => {
            ...
            diet = await dietsCollection.create(d => {
                ...
                d.restrictions = dietData.restrictions || [];
                d.isActive = true;
            });
        });
        return diet!;
