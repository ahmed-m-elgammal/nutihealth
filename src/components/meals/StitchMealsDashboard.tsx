...
export default function MealsDashboard() {
    ...
    const [pendingDeleteFood, setPendingDeleteFood] = useState<FoodListItem | null>(null);
    ...
    const { handleDeleteFood, handleDuplicateFood, handleEditFood, handleAddNote } = useMealActions({
        router,
        showToast,
    });
    ...
    const grouped = useMemo(() => {
        ...
        foodsForDay.forEach((food) => {
            if (init[food.mealType]) init[food.mealType].push(food);
        });
        return init;
    }, [foodsForDay]);
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#0f172a' }}>
            ...
            >
                ...
                <View style={{ backgroundColor: '#1e293b', borderRadius: 24, padding: 24, marginBottom: 28 }}>
                    <Text style={{ color: '#f8fafc', fontSize: 16, fontWeight: '700', marginBottom: 20 }}>
                        Daily Calories
                    </Text>
                    ...
                    <View style={{ gap: 12 }}>
                        {MEAL_TYPES.map((mealType) => {
                            ...
                            return (
                                <View
                                    key={mealType}
                                    style={{ backgroundColor: '#1e293b', borderRadius: 20, overflow: 'hidden' }}
                                >
                                    {/* Meal Header */}
                                    <Pressable
                                    ...
                                    >
                                        <View>
                                            ...
                                            <Text style={{ color: '#94a3b8', fontSize: 12, marginTop: 2 }}>
                                                {items.length === 0
                                                    ? 'Not logged yet'
                                                    : `${items.length} item${items.length > 1 ? 's' : ''} · ${mealCalories} kcal`}
                                            </Text>
                                        </View>
                                        ...
                                        <View style={{ paddingHorizontal: 12, paddingBottom: 12, gap: 6 }}>
                                            {items.map((food) => (
                                                <FoodItemCard
                                                    ...
                                                    onDelete={() => setPendingDeleteFood(food)}
            ...
            <ConfirmationSheet
                visible={Boolean(pendingDeleteFood)}
                title="Delete meal item"
                message={pendingDeleteFood ? `Remove ${pendingDeleteFood.name} from your log?` : ''}
                confirmLabel="Delete"
                cancelLabel="Cancel"
                onCancel={() => setPendingDeleteFood(null)}
                onConfirm={() => {
                    const target = pendingDeleteFood;
                    setPendingDeleteFood(null);
                    if (!target) return;
                    handleDeleteFood(target).catch(() => undefined);
                }}
                destructive
            />
