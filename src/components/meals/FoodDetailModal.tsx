...
type FoodDetailModalProps = {
    ...
    onSave: (payload: { quantity: number; note: string }) => void;
    onRemove: () => void;
};
...
export default function FoodDetailModal({
    ...
    return (
        ...
        >
            ...
            <View style={{ marginTop: 14 }}>
                <Text style={{ color: '#334155', marginBottom: 6 }}>Note</Text>
                <TextInput
                    value={note}
                    onChangeText={setNote}
                    placeholder="Optional note"
                    autoFocus={autoFocusNote}
                    style={{ borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 12, padding: 12 }}
                />
            </View>

            <View style={{ marginTop: 18, flexDirection: 'row', gap: 10 }}>
                <Pressable
                    onPress={() => onSave({ quantity: Math.max(0.1, q), note: note.trim() })}
