...
function ExerciseList({ exercises, onMoveExercise, onUpdateSet }: ExerciseListProps) {
    const keyExtractor = useCallback((item: TrackerExercise) => item.id, []);
    const renderItem = useCallback(
        ({ item, index }: { item: TrackerExercise; index: number }) => (
            <ExerciseCard item={item} index={index} onMoveExercise={onMoveExercise} onUpdateSet={onUpdateSet} />
        ),
        [onMoveExercise, onUpdateSet],
    );

    return <FlashList data={exercises} estimatedItemSize={160} keyExtractor={keyExtractor} renderItem={renderItem} />;
}
