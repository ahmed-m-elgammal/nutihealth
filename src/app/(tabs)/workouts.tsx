...
export default function WorkoutsScreen() {
    ...
    const [schedulePreferences, setSchedulePreferences] = useState<WorkoutSchedulePreferences>(
        sanitizeSchedulePreferences()
    );
    ...
    const handleSelectStartDay = (dayOfWeek: WorkoutSchedulePreferences['startDay']) => {
        setSchedulePreferences((previous) => sanitizeSchedulePreferences({
            ...previous,
            startDay: dayOfWeek,
        }));
    };
    ...
    const handleApplySchedulePreferences = async () => {
        ...
        try {
            ...
            const templateIds = Array.from(
                new Set(schedules.map((schedule) => schedule.templateId).filter(Boolean))
            );
    ...
    return (
        <SafeAreaView className="flex-1 bg-neutral-50" edges={['top']}>
            <View className="bg-white px-6 py-4 border-b border-neutral-100 shadow-sm flex-row items-center justify-between">
                <View>
                    <Text className="text-2xl font-bold text-neutral-900">Workouts</Text>
                    <Text className="text-neutral-500 text-sm mt-1">Your Weekly Schedule</Text>
                </View>
                <TouchableOpacity onPress={handleOpenPrograms} className="bg-indigo-50 p-2 rounded-full">
            ...
            <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                {isLoadingPlan ? (
                    <View className="p-6 items-center">
                        ...
                        <Text className="text-neutral-500 mt-3">Loading schedule...</Text>
                    </View>
                ) : weeklyPlan ? (
                    <WeeklyCalendar
                        plan={weeklyPlan}
                ...
                {!isLoadingPlan && weeklyPlan && (
                    <View className="px-6 mt-1">
                        <View className="bg-white border border-neutral-100 rounded-2xl p-4">
                            <Text className="text-neutral-900 font-semibold mb-1">Schedule Controls</Text>
                            ...
                            <Text className="text-neutral-700 font-medium text-xs uppercase mb-2">Start Day</Text>
                                ...
                                {WEEK_DAYS.map((dayOfWeek) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            className={`px-3 py-2 rounded-full mr-2 mb-2 border ${isSelected
                                                ? 'bg-indigo-600 border-indigo-600'
                                                : 'bg-white border-neutral-200'
                                                }`}
                                        >
                                            <Text className={`text-xs font-semibold ${isSelected ? 'text-white' : 'text-neutral-600'}`}>
                                                {dayOfWeek.slice(0, 3)}
                                            </Text>
                                        </TouchableOpacity>
                                    );
                            ...
                            <Text className="text-neutral-700 font-medium text-xs uppercase mb-2">Rest Days</Text>
                            <View className="flex-row flex-wrap mb-3">
                                {WEEK_DAYS.map((dayOfWeek) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            className={`px-3 py-2 rounded-full mr-2 mb-2 border ${isRestDay
                                                ? 'bg-amber-100 border-amber-300'
                                                : 'bg-white border-neutral-200'
                                                }`}
                                        >
                                            <Text className={`text-xs font-semibold ${isRestDay ? 'text-amber-800' : 'text-neutral-600'}`}>
                                                {dayOfWeek.slice(0, 3)}
                                            </Text>
                                        </TouchableOpacity>
                                    );
                            ...
                            <TouchableOpacity
                                ...
                                className={`py-3 rounded-xl items-center ${isApplyingSchedule ? 'bg-neutral-300' : 'bg-indigo-600'}`}
                            >
                ...
                {!isLoadingPlan && weeklyPlan && !hasScheduledDays && (
                    <View className="px-6 mt-1">
                        <View className="bg-white border border-neutral-100 rounded-2xl p-4">
                            <Text className="text-neutral-900 font-semibold mb-1">No active program selected</Text>
                            <Text className="text-neutral-500 text-sm mb-3">
                            ...
                            >
                                <Text className="text-white font-semibold">Browse Programs</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
                ...
                <View className="px-6 mt-4">
                    <Text className="text-neutral-900 font-bold text-lg mb-4">Quick Actions</Text>
                    ...
                    <View className="flex-row gap-4 mb-4">
                        ...
                        >
                            <View className="bg-neutral-100 self-start p-3 rounded-xl mb-3">
                                <Plus size={24} color="#171717" />
                            </View>
                            <Text className="text-neutral-900 font-bold text-base">Quick Log</Text>
                            <Text className="text-neutral-500 text-xs">Record manually</Text>
                        </TouchableOpacity>

                        <TouchableOpacity
                            className="flex-1 bg-indigo-600 rounded-2xl p-4 shadow-sm border border-indigo-600"
                            onPress={handleOpenPrograms}
                        >
                            <View className="bg-indigo-500 self-start p-3 rounded-xl mb-3">
                                <Dumbbell size={24} color="#FFFFFF" />
                            </View>
                            <Text className="text-white font-bold text-base">Browse Library</Text>
                            <Text className="text-indigo-100 text-xs">Structured Programs</Text>
                        </TouchableOpacity>
                    </View>
